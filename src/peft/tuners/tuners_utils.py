...
class BaseTuner(nn.Module, ABC):
    ...
    def inject_adapter(self, model: nn.Module, adapter_name: str):
        ...
        #adapter_layer= [1,2,3,4,5,6,7,8]
        adapter_layer= 'all'
        activate_adapter_layers = ['model.layers.'+ str(key) +'.' for key in adapter_layer] if adapter_layer!='all' else True
        # update peft_config.target_modules if required
        peft_config = _maybe_include_all_linear_layers(peft_config, model)
        for key in key_list:
            flag_adapter_layers = True if adapter_layer=='all' else any(flag in key for flag in activate_adapter_layers)
            if not flag_adapter_layers:
                continue
