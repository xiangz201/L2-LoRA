...
class Trainer:
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        if (not isinstance(model, DistributedDataParallel)) and (not isinstance(model, DeepSpeedEngine)) and (
        not isinstance(model, LlamaForCausalLM)) and (not isinstance(model, PeftModelForCausalLM)):
            print(type(model))
            print(model)
            raise ValueError("Model type not setup.")
        NAIE = [0.050669584423303604, 0.05638622120022774, 0.05862056091427803, 0.05963214859366417, 0.0590033084154129,
                0.05819687992334366, 0.05721728131175041, 0.05522366985678673, 0.05508431792259216, 0.05203869566321373,
                0.048177581280469894, 0.044918522238731384, 0.04171902313828468, 0.03899724408984184,
                0.035897038877010345, 0.03178909793496132, 0.024329356849193573, 0.021856505423784256,
                0.01854489929974079, 0.017490793019533157, 0.014829898253083229, 0.014242724515497684,
                0.01305896881967783, 0.011700465343892574, 0.010230395942926407, 0.00964277982711792,
                0.007196357939392328, 0.006935243494808674, 0.007760652806609869, 0.008408168330788612,
                0.005837664473801851, 0.004363934509456158]
        # cubic decay schedule
        def compute_cubic_naie(idx, L, tau):
            ell_i = L / 3
            ell_f = 2 * L / 3
            ...
            if idx < ell_i:
                ...
                return tau + (1 - tau) * (1 - ((idx - ell_i) / (ell_f - ell_i)) ** 3)
            else:
                return tau
        if isinstance(model, DistributedDataParallel):
            for idx, decoder_layer in enumerate(model.module.base_model.model.model.layers):
                flag_name = decoder_layer.self_attn.q_proj.active_adapter[0]
                l2_value = 0
                if 'lora_A' in [key for key, _ in decoder_layer.self_attn.q_proj.named_modules()]:
                    l2_value += decoder_layer.self_attn.q_proj.lora_A[flag_name].weight.pow(2) + \
                            decoder_layer.self_attn.q_proj.lora_B[flag_name].weight.pow(2)
                if 'lora_A' in [key for key, _ in decoder_layer.self_attn.k_proj.named_modules()]:
                    l2_value += decoder_layer.self_attn.k_proj.lora_A[flag_name].weight.pow(2) + \
                            decoder_layer.self_attn.k_proj.lora_B[flag_name].weight.pow(2)
                if 'lora_A' in [key for key, _ in decoder_layer.self_attn.v_proj.named_modules()]:
                    l2_value += decoder_layer.self_attn.v_proj.lora_A[flag_name].weight.pow(2) + \
                            decoder_layer.self_attn.v_proj.lora_B[flag_name].weight.pow(2)
                if 'lora_A' in [key for key, _ in decoder_layer.mlp.gate_proj.named_modules()]:
                    l2_value += decoder_layer.mlp.gate_proj.lora_A[flag_name].weight.pow(2) + \
                            decoder_layer.mlp.gate_proj.lora_B[flag_name].weight.pow(2)
                if 'lora_A' in [key for key, _ in decoder_layer.mlp.up_proj.named_modules()]:
                    l2_value += decoder_layer.mlp.up_proj.lora_A[flag_name].weight.pow(2) + \
                            decoder_layer.mlp.up_proj.lora_B[flag_name].weight.pow(2)
                if 'lora_A' in [key for key, _ in decoder_layer.mlp.down_proj.named_modules()]:
                    l2_value += decoder_layer.mlp.down_proj.lora_A[flag_name].weight.pow(2) + \
                            decoder_layer.mlp.down_proj.lora_B[flag_name].weight.pow(2)

                l2_value = l2_value * NAIE[idx]
                # l2_value = l2_value * compute_cubic_naie(idx, 32, 0.1)
                outputs['loss'] += 0.1 * l2_value.to(outputs['loss'].dtype).view(outputs['loss'].shape)

        if isinstance(model, DeepSpeedEngine):
            for idx, decoder_layer in enumerate(model.module.model.layers):
                flag_name = decoder_layer.self_attn.q_proj.active_adapter[0]
                l2_value = 0
                if 'lora_A' in [key for key, _ in decoder_layer.self_attn.q_proj.named_modules()]:
                    l2_value += decoder_layer.self_attn.q_proj.lora_A[flag_name].weight.pow(2) + \
