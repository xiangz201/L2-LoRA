...
class HFLM(TemplateLM):
    ...
    def tok_encode(
        ...
        '''
        print('specical_tokens_kwargs',special_tokens_kwargs)
        print('encoding',encoding)
        tokens = self.tokenizer.convert_ids_to_tokens(encoding)

        # 打印每个token ID及其对应的词或子词
        for token_id, token in zip(encoding, tokens):
    ...
    def generate_until(
        self, requests: List[Instance], disable_tqdm: bool = False, task_flag='ori'
    ) -> List[str]:
        res = []
