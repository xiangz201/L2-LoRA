...
class ConfigurableTask(Task):
    ...
    def construct_requests(
        ...
        if self.OUTPUT_TYPE == "loglikelihood":
            ...
            # generate LKC raw dataset
            if self.config['task']=='winogrande':
                gold_result = self.doc_to_text(doc)
                gold_result = doc['option1'] if gold_result==0 else doc['option2']
            elif self.config['task']=='agieval_aqua_rat':
                gold_result = self.doc_to_target(doc)
                gold_result = self.doc_to_choice(doc)[gold_result[0]]
            elif self.config['task']=='openbookqa' or self.config['task']=='social_iqa' or self.config['task']=='arc_easy' or self.config['task']=='arc_challenge':
                gold_result = self.doc_to_target(doc)
                gold_result = self.doc_to_choice(doc)[gold_result]
            else:
                gold_result = self.doc_to_target(doc)
                gold_result = self.doc_to_choice(doc)[gold_result]  
            gold_result = (f"{target_delimiter}{gold_result}",self.config['task'])
            ...
            #arguments = (ctx, deepcopy(self.config.generation_kwargs), doc['answer'].split('\n')[-1])
            arguments = (ctx, deepcopy(self.config.generation_kwargs))              #humaneval
        multimodal_arg = {}
        if (
