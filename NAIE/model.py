...
class TemplateLM(LM):
    ...
    def loglikelihood(
        self, requests, disable_tqdm: bool = False, task_flag='ori'
        ...
        for context, continuation  in [req.args for req in requests]:
            if context == "":
                ...
                #print(self.tokenizer.decode(context_enc))
                #print(self.tokenizer.decode(continuation_enc))
                # 打印每个token ID及其对应的词或子词
                #print(continuation)
                #tokens = self.tokenizer.convert_ids_to_tokens(continuation_enc)
                #for token_id, token in zip(continuation_enc, tokens):
                #    print(f"Token ID: {token_id}, Token: {token}")
                #print(dfa)
            new_reqs.append(((context, continuation), context_enc, continuation_enc))
        ''' special used for gsm8k, math, humaneval, mbpp
        for req in requests:
            context, _  = req.args
            #continuation = req.doc['answer'] # gsm8k
            #continuation = req.doc['solution']  #math
            continuation = req.doc['canonical_solution']  #humaneval
            #continuation = req.doc['code']   #mbpp
            if context == "":
                # BOS or EOS as context
                context_enc, continuation_enc = (
                    [self.prefix_token_id],
        ...
        '''
        if task_flag=='ori':
            print('******run original [lm-eval] repo, check the [task_flag] in your code******')
            return self._loglikelihood_tokens(new_reqs, disable_tqdm=disable_tqdm)
        elif task_flag=='causal_tracing':
            from lm_eval.api.rome_main import calculate_hidden_flow
            from lm_eval.api.rome_main import plot_trace_heatmap
            import torch
            import numpy
            from lm_eval.api.rome_main import collect_embedding_std
            noise_level = 3.0 * collect_embedding_std(
                    self, new_reqs)
            flag_id=0
            for i,reqs in enumerate(new_reqs):
                if requests[0].metadata[0]=='gsm8k' or 'hendrycks' in requests[0].metadata[0] or requests[0].metadata[0]=='humaneval' or requests[0].metadata[0]=='mbpp':
                    knowns.append((reqs, flag_id, requests[0].metadata[0]))
                    flag_id+=1
                elif requests[0].gold_result[1]=='winogrande':
                    if reqs[0][0].endswith(requests[i].gold_result[0].replace(' ','',1)):
                        knowns.append((reqs, flag_id, requests[i].gold_result[1]))
                        flag_id+=1
                elif requests[0].gold_result[1]=='arc_easy' or requests[0].gold_result[1]=='arc_challenge' or requests[0].gold_result[1]=='social_iqa' or requests[0].gold_result[1]=='openbookqa':
                    if reqs[0][-1].endswith(requests[i].gold_result[0].replace(' ','',1)):
                        knowns.append((reqs, flag_id, requests[i].gold_result[1]))
                        flag_id+=1
                else:
                    if reqs[0][-1] == requests[i].gold_result[0]:
                        knowns.append((reqs, flag_id, requests[i].gold_result[1]))
                        flag_id+=1
            result_dir = './results_llama3/humaneval/cases'+'_'+requests[0].metadata[0]
            pdf_dir = './results_llama3/humaneval/pdfs'+'_'+requests[0].metadata[0]
            os.makedirs(result_dir, exist_ok=True)
            os.makedirs(pdf_dir, exist_ok=True)
            # load Domain-Specific Knowledge Localization Datasets, include annotated words to be interfered with
            filename = 'DoLoD/hellaswag_annot.json'
            with open(filename, "r", encoding="utf-8") as f:
                data_erange_all = json.load(f)
            for knowledge in tqdm(knowns2):
                known_id = knowledge[1]
                kind=None
                for kind in None, "mlp", "attn":
                    data_erange=data_erange_all[known_id]
                    ...
                    if not os.path.isfile(filename):
                        ...
                        if result==None:
                            tqdm.write(f"Skipping knowledge")
                            continue
                        numpy_result = {
                    ...
                    plot_trace_heatmap(plot_result, savepdf=pdfname,modelname='LLaMA2-7B')
            print('******Finish LKC, layer-wise results saved in [results_llama3]******')
